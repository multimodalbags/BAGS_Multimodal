...
import utils
from train import instance_bce_with_logits
...
def evaluate(model, dataloader, dataroot, target_ans=None, verbose=False, show_top=False):
    ...
    for v, b, q, a, q_id in iter(dataloader):
        ...
        total_loss += instance_bce_with_logits(pred, a).data * v.size(0)
        ...
        for i in range(batch_size):
            idx = int(pred_max[i])
            occ[idx] += 1
            pasr_hit[idx] += np.array((a[i, idx] == 0.0).cpu())
            pasr_possible += np.array((a[i,:] == 0.0).cpu())
    ...
    return score, upper_bound, asr, pasr, pasr_ans, total_loss
...
def evaluation_suite(model, dataroot, batch_size, ver='clean', target_ans=None, saveroot=None):
    ...
    eval_loader =  DataLoader(eval_dset, batch_size, shuffle=True, num_workers=16)
    score, _, asr, _, _, _ = evaluate(model, eval_loader, dataroot, target_ans, verbose=True)
    ...
    if ver != 'clean':
        ...
        score, _, asr, _, _, loss = evaluate(model, eval_loader, dataroot, target_ans, verbose=True, show_top=True)
        summary_lines.append("troj  \t%.4f\t%.4f"%(score, asr))
        total_loss += loss
        sample_num += len(eval_loader.dataset)
        ...
        eval_loader =  DataLoader(eval_dset, batch_size, shuffle=True, num_workers=4)
        score, _, io_asr, _, _, loss = evaluate(model, eval_loader, dataroot, target_ans, verbose=True)
        summary_lines.append("troj_i\t%.4f\t%.4f"%(score, io_asr))
        total_loss += loss
        sample_num += len(eval_loader.dataset)
        ...
        eval_loader =  DataLoader(eval_dset, batch_size, shuffle=True, num_workers=4)
        score, _, qo_asr, _, _, loss = evaluate(model, eval_loader, dataroot, target_ans, verbose=True)
        summary_lines.append("troj_q\t%.4f\t%.4f"%(score, qo_asr))
        total_loss += loss
        sample_num += len(eval_loader.dataset)
    ...
    if saveroot is not None:
        ...
        with open(save_file, 'w') as f:
            for line in summary_lines:
                f.write(line+'\n')
    return asr, io_asr, qo_asr, total_loss
...
def eval_vqa(args):
    ...
    eval_dset = VQAFeatureDataset('val', dictionary, extra_iter=True, verbose=False,
                                dataroot=args.dataroot, ver=args.data_id,
    ...
    if args.full: # run full evaluation suite
        asr, io_asr, qo_asr, loss = evaluation_suite(model, args.dataroot, 256, args.data_id, args.target, saveroot=None)
