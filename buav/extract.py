...
import base_model
from train import train, compute_score_with_logits, instance_bce_with_logits
import utils
...
def extract(model, dataloader, dataroot, results_path):
    ...
    for v, b, q, a, q_id in tqdm.tqdm(iter(dataloader)):
        ...
        total_loss += instance_bce_with_logits(pred, a).data
...
def extract_suite(model, dataroot, batch_size, ver, model_id, resdir, detector, nb):
    ...
    for tc in trojan_configs:
        ...
        loss = extract(model, eval_loader, dataroot, results_path)
        if tc != 'clean':
            r_loss += loss
    return r_loss
